...
We introduced JavaScript Objects when examining the global window object. Let's review some basic syntax for Objects:

- JavaScript Objects are scoped with `{ }`, and consist of collections of **key-value pairs**.
...
> **NOTE**: With **JavaScript Objects**, we _don't have to_ put quotation marks around our keys because no matter what JS will expect those to be Strings.
...
Just like Arrays, we can access items inside an Object. Instead of accessing values by index, we access values by key. We can access data inside Objects by using either dot notation when the key is known and can be hardcoded or bracket notation when the key unknown and will be determine programmically.

So, with bracket notation, what's the difference with using quotation marks or not? Well, recall that quotation marks make some text behave as a string, whereas the absence of quotation marks means that JavaScript is expecting to find a variable of that name. 

Using the above movie object as an example, let's determine if it has a cast property (key):

```javascript
if (movie.cast) {
  movie.cast.forEach(member => {
    console.log(`Cast member: ${member}`);
  });
}
...
### **Determine if a property exists on an object**

Here are a few ways to check if a property exists on a JavaScript object using the in operator:
...
// Check for 'hello' property  
if (!('hello' in user)) {
  console.log('user does NOT have hello property');
}
...
The in operator will check if the property exists on the object itself or anywhere in its prototype chain. To check just on the object itself, use hasOwnProperty() or Object.keys().

The main idea is to use in or hasOwnProperty() to check if the property exists on the given object. The in operator is the most concise way to check for a property existence.

[MDN In Operator Documentation](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/in)
...
> Note: Although both the `in` operator and `for...in` interation use the same keyword `in`, they have different purposes and should not be confused.
...
// create an object called "obj" that contains 3 properties
// each property contains a value 
for (const prop in obj) {
// create a for in loop that iterates over the "obj" object 
